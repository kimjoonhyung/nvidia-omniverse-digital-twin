> 🇰🇷 한국어 | 🇺🇸 [English](en/04-streaming-viewer.md)

# 04. 스트리밍 뷰어 — 노트북으로 트윈 관전·조작 (선택)

> **← 이전:** [03. 실시간 데이터](03-실시간-데이터.md) &nbsp;|&nbsp; **처음으로:** [코스 안내](../README.md)
>
> **목표**: 배포된 Isaac Sim 클라이언트 3대 중 **1대를 "스트리밍 호스트"**로 만들고,
> 나머지 뷰어는 **자기 노트북의 데스크톱 앱**으로 그 트윈을 실시간으로 본다.
...
```mermaid
flowchart LR
    HOST["<b>클라이언트 #3 · 스트리밍 호스트</b><br/>isaac-sim.streaming.sh (GPU 렌더 + WebRTC)<br/>+ robot.monitor 확장 + 공장 씬"]
    V["<b>뷰어들 · 데스크톱 앱</b><br/>Isaac Sim WebRTC Streaming Client<br/>(Win / macOS / Linux)"]
    HOST -->|"49100/TCP 시그널"| V
    HOST -->|"47998/UDP 미디어"| V
    V -.->|"마우스·키보드 입력"| HOST
...
- **`allowCidr`**: 관리 접근(DCV·SSH) 허용 IP — 관리자 IP `/32`. (필수)
- **`viewerCidr`**: 뷰어(WebRTC 접속) 허용 대역 — 뷰어들이 접속하는 IP 대역.
...
## STEP 3. 뷰어 노트북에 네이티브 클라이언트 설치

각 뷰어는 **Isaac Sim WebRTC Streaming Client** 데스크톱 앱을 받는다.
