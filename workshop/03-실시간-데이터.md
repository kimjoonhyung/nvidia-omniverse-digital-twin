> 🇰🇷 한국어 | 🇺🇸 [English](en/03-live-data.md)

# 03. 실시간 데이터 — 살아 움직이는 트윈

> **← 이전:** [02. 협업](02-협업-Nucleus-Live.md) &nbsp;|&nbsp; **다음 →** [04. 스트리밍 뷰어](04-스트리밍-뷰어.md)
>
...
```mermaid
flowchart LR
    SIM["factory_simulator.py<br/>로봇 4종 가짜 데이터"]
    IOT["AWS IoT Core"]
    KIN["Kinesis<br/>robot-telemetry"]
    ISAAC["Isaac Sim + robot.monitor 확장<br/>실시간 차트 · 로봇 이동 · 클릭 대시보드"]
...
```
> `~/venv/bin/python` 을 써야 한다(시스템 python 엔 awsiotsdk 없음). 상세는 [`../docs/iot-개발노트.md`](../docs/iot-개발노트.md) 2장.
...
새 터미널에서 **모니터링 확장**을 얹어 실행한다 (다중 사용자 환경 기준 — 포트 자동 분리):
```bash
launch-isaac --ext-folder ~/digital_twin/exts --enable robot.monitor
```

> `launch-isaac` 을 쓰는 이유·직접 실행(`/opt/IsaacSim/isaac-sim.sh`) 방법은
> [00. 시작하기](00-시작하기.md) STEP 2 참고. 여기에 `--ext-folder ... --enable robot.monitor`
> 를 붙여 실시간 모니터링 확장(`robot.monitor`)을 함께 켠다.
...
여러 명이 같은 화면을 보려면 [02. 협업 — Nucleus Live](02-협업-Nucleus-Live.md) 를 함께 쓴다.
- 한 명이 **Publish ON** 으로 데이터를 USD 에 기록.
...
# Thing/인증서/정책은 detach 후 삭제 (순서 주의) — 자세히는 ../docs/iot-개발노트.md
```
...
| 연결 "unable to connect" | Nucleus 의 SERVER_IP 공란 문제 → [`../docs/iot-개발노트.md`](../docs/iot-개발노트.md) 트러블슈팅 |
| 인증서 에러 | `certs/` 에 device.cert.pem/private.key/AmazonRootCA1.pem 3개 있는지 |

상세 기술 배경은 [`../docs/iot-개발노트.md`](../docs/iot-개발노트.md), 확장 구조는 `../exts/robot.monitor/` 참고.

---

**다음 →** [04. 스트리밍 뷰어 (선택)](04-스트리밍-뷰어.md)
