...
class RobotState:
    ...
    def step(self, dt: float) -> dict:
        ...
        if self.status == "moving":
            ...
            self.battery -= dt * random.uniform(0.05, 0.12)   # 소모
            self.motor_temp += dt * random.uniform(0.05, 0.2) # 발열
            # 위치 이동
            ...
            if random.random() < 0.02:      # 가끔 에러 이벤트
                self.errors += 1
            if self.battery <= 20:
...
def main():
    ...
    ap.add_argument("--robots", type=int, default=1, help="로봇 대수")
    ap.add_argument("--interval", type=float, default=5.0, help="publish 간격(초)")
    args = ap.parse_args()
