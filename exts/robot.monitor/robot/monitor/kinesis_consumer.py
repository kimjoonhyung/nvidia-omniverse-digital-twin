...
(English)
Kinesis telemetry consumer (background thread).

Polls Kinesis on a separate thread so the Isaac Sim UI is never blocked, and keeps
per-robot latest values + time-series buffers (deques) in a thread-safe manner.

No Isaac Sim dependency → can be tested standalone (see __main__ below).
"""
import base64
import json
import threading
...
MAX_POINTS = 120  # 로봇별 시계열 보관 길이 (5초 간격이면 10분) / per-robot series length (10 min at 5s intervals)


class TelemetryStore:
    """로봇별 최신값 + 시계열. 모든 접근은 lock 으로 보호.

    Per-robot latest values + time series. All access is protected by a lock.
    """
...
class KinesisConsumer:
    ...
    Background thread that polls the Kinesis stream and fills the TelemetryStore.
    """
    ...
    def _run(self):
        try:
            ...
            # Only new records from the start time — LATEST (skips already-accumulated history)
...
if __name__ == "__main__":
    ...
    # Standalone test: verify Kinesis consumption works without Isaac Sim
