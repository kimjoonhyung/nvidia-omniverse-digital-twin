...
class RobotMonitorExtension(omni.ext.IExt):
    def on_startup(self, ext_id):
        self._store = TelemetryStore()          # 발행 모드일 때 Kinesis 시계열
        self._consumer = KinesisConsumer(self._store)
        self._publish = True                     # 발행 모드 토글 (PoC: 기본 ON)
        self._consumer.start()                   # 시작 시 바로 소비
        self._last_ts = {}                        # robot_id → 마지막 처리한 ts (중복 방지)
        self._move_robots = True                  # 텔레메트리 position 으로 로봇 이동
        self._motion = {}                         # robot_id → {cur:[x,y,h], tgt:[x,y,h]} 보간 상태
        self._display_series = {}                # 표시 모드 시계열(USD에서 읽은 값 누적)
