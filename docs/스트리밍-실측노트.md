> 🇰🇷 한국어 | 🇺🇸 [English](en/streaming-field-notes.md)
...
> **DCV virtual 다중세션**으로 해결(자동화 완료 — 4장). 아래는 근거와 시행착오 전체.
...
- 이 프로젝트에 **이미 구축·검증됨**: [`../workshop/02-협업-Nucleus-Live.md`](../workshop/02-협업-Nucleus-Live.md), [`iot-개발노트.md`](iot-개발노트.md) Live 공유 원리.
...
### 배포 클라이언트(마켓플레이스 AMI) 초기 현황 — 실측
- `nice-dcv-server` 2025.0 설치됨, **console 세션 1개**로 구성(`automatic-console-session`).
- **`dcv-gl` 미설치**(apt 저장소에도 없음, NICE에서 수동 설치), `Xdcv`도 안 보임.
- → virtual 다중세션 + GPU 가속을 쓰려면 추가 설치·설정 필요 → **아래 자동화로 해결(완료)**.

### DCV virtual 다중세션 자동 세팅 (✅ 완료 — CDK user-data)

`cdk-omniverse/lib/client-userdata.ts` 의 systemd oneshot 서비스 `dcv-multiuser` 가
부팅 시 아래를 멱등하게 수행한다. **L40S 1대에 student 8명 + Isaac Sim GPU 가속 동시 접속 실증.**

1. **`nice-xdcv` + `nice-dcv-gl` 설치** — virtual 세션 X 서버(Xdcv, 없으면 세션 즉사)와
   GPU 오프로드 런타임(dcvgladmin/dcvgldiag). 마켓플레이스 AMI 엔 없어 공식 tgz 에서 설치.
2. **console 자동생성 끄기** — `create-session = false` + 기존 console 세션 close.
   (console 과 virtual 은 동시 불가.)
3. **GPU Xorg 를 `DISPLAY=:0` 에 정렬** — DCV-GL 은 GL 호출을 ":0 의 3D X 서버"로 오프로드한다.
   gdm 의 GPU Xorg 가 `:1` 로 밀리거나 :0 이 virtual 세션 자신이면 **llvmpipe(SW 렌더) 폴백**돼
   Isaac Sim 이 안 돈다. stale X 소켓/락 제거 후 `dcvgldiag` 가 **"No problem found"** 일 때까지 폴링·검증.
4. **student1..N 계정 생성 + 계정별 virtual 세션 생성** — 같은 OS 유저로 다중 세션은 홈/D-Bus
   충돌 → **계정을 분리**한다. 공통 비밀번호는 `StudentPassword`(빈 값이면 랜덤 후 로그 기록).
5. **deb 브라우저(epiphany) + `launch-isaac` 런처 배치** — AMI 의 snap firefox/chromium 은
   virtual 세션에서 mount namespace 충돌로 실행 불가. Navigator 는 `epiphany` 로 연다.
   `launch-isaac` 은 uid 기준으로 Isaac Sim HTTP 포트를 분리(student1→8001 … 8→8008)해
   동시 실행 시 `address already in use` 를 막는다.
6. (선택) 각 세션이 **Nucleus Live** 참여 → 1 GPU 여러 명 + **같은 트윈** 공유.

> 상세 파라미터·배포 명령은 [`../cdk-omniverse/README.md`](../cdk-omniverse/README.md) "다중 사용자 접속" 절 참고.

---

## 5. 실배포 참고값 (2026-07, ap-northeast-2)

> 실제 IP·비밀번호는 배포 때마다 다르므로 저장소에 두지 않는다. 형식만 기록.
...
| Nucleus 사설 IP (Isaac 연결) | `<Nucleus-사설IP>` (CDK Outputs 참고) |
