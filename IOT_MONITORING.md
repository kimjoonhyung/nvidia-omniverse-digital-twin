...
[factory_simulator.py]──MQTT──▶ [AWS IoT Core] ──IoT Rule──▶ [Kinesis: robot-telemetry]
   (로봇 4종 발행. 단일 테스트는 robot_simulator.py)                 │
                                                                     ▼
...
| 스크립트 | 발행 로봇 | 용도 |
|----------|-----------|------|
| `robot_simulator.py` | `nova_carter_01..N` (1종, `--robots N` 로 대수 조절) | 파이프라인/단일 로봇 테스트 |
| **`factory_simulator.py`** | **4종**: `nova_carter_01`(amr), `iw_hub_01`(amr), `franka_01`(arm), `digit_01`(humanoid) | **워크샵 공장 씬(factory_scene) 데모** |

> 워크샵에서 로봇 4종이 화면에 다 나와야 하면 **`factory_simulator.py`** 를 써야 한다.
> `robot_simulator.py` 는 nova_carter 만 발행하므로 목록에 nova_carter_0N 만 뜬다.
...
실행 (venv 파이썬 사용):
```bash
# 워크샵 4종 (권장) — nova_carter/iw_hub/franka/digit
IOT_ENDPOINT=$(cat ~/.iot_endpoint) ~/venv/bin/python -u factory_simulator.py --interval 3

# 또는 단일 로봇 테스트 (nova_carter 만)
IOT_ENDPOINT=$(cat ~/.iot_endpoint) ~/venv/bin/python -u robot_simulator.py --interval 5      # 1대
IOT_ENDPOINT=$(cat ~/.iot_endpoint) ~/venv/bin/python -u robot_simulator.py --robots 3 --interval 5   # 3대
```
- 셋업이 `~/.bashrc` 에 `IOT_ENDPOINT` 를 넣으므로, 새 셸에선 `~/venv/bin/python -u factory_simulator.py ...` 만 해도 된다.
